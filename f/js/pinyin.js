(function(global) {
  var pinyinDB = {
    "a": {
      ...
      "cun": ".N2nA>lS",
      "lü": "$()(*r+~0`5Z5~6S7_7j9q:*@wA(A8A;HkM,NKV=VZm'rJw#xDz_{T-*u-+*-5a",
    ...
    "m": {
      ...
      "lü": "-6F-63-4#-38-23-,'-*x-(t-(F-&G|+nTnSnPl(e^`A`5ZcZ*YwS.K*HTDsDoAYA)9M6D3A0]+I",
  ...
  var DB = window.DB = {},
    ...
    fromX = function(str) {
      ...
      for (var index = str.length - 1; index > -1; index--) {
        result += temp * (chars.indexOf(str.charAt(index)));
        temp *= 91;
      }
      return result;
    },
    makeDB = function(data, f) {
      var pinyin, groupString, i, chinese, num;
      for (pinyin in data) {
        if (data.hasOwnProperty(pinyin)) {
          groupString = data[pinyin].match(group);
          for (i = 0; i < groupString.length; i++) {
            chinese = groupString[i].replace(replaceGroup, '#');
            num = fromX(chinese);
            chinese = String.fromCharCode(base + middle + (f ? -num : num));
            if (DB.hasOwnProperty(chinese)) {
              DB[chinese].push(pinyin);
            } else {
              DB[chinese] = [pinyin];
            }
          }
  ...
  pinyinDB = null;
  global.pinyinUtil = {
    toPinyin: function(chinese) {
      var pinyin = [];
      for (var i = 0; i < chinese.length; i++) {
        if (DB.hasOwnProperty(chinese[i])) {
          pinyin.push(DB[chinese[i]]);
        } else {
          pinyin.push(chinese[i])
        }
      }
      return pinyin;
    }
